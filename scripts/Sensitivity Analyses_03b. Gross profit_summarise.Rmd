---
title: "Sensitivity analysis - Summarise economic data"
author: "Alexa Varah"
date: "2023-01-09"
...
# 2. Load data
```{r load-data}
econ_allimp_SA <- readRDS(file = "../output/sensitivity analyses/economic_data_for_analysis-all_imputations_SA.rds")
```
...
```{r}
summarised_df_3DR_SA <- econ_allimp_SA %>% 
...
econ_distinct_3DR_SA <- econ_allimp_SA %>% 
  select(sensanaly, ID, crop, fertcost, seedcost, herbcost, sundry, varcost) %>% 
  distinct(sensanaly, ID, .keep_all=TRUE)

# do the join
econ_3DR_SA <- inner_join(econ_distinct_3DR_SA, summarised_df_3DR_SA,  
                       by = c("ID","sensanaly")) %>% 
  dplyr::mutate(
  ...
  dplyr::arrange(scenario, initdenres_collapsed, region, soiltype, sensanaly) %>% 
  dplyr::relocate(sensanaly)

# Clean up
rm(econ_distinct_3DR_SA, summarised_df_3DR_SA)
```

## 3.2. Save
```{r}
write.csv(econ_3DR_SA,
          '../output/sensitivity analyses/economic_data_for_analysis_3DR_SA.csv',
          row.names = FALSE)
```

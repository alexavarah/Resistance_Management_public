...
# Fig 5: Scatter plot - tipping point
oc_fig5 <- read.csv("../output/data_for_Figure_5.csv", 
                     header = T) %>% 
  dplyr::mutate(
    region = factor(region, levels = c("north", "central", "east")),
    soiltype = factor(soiltype, levels = c("heavy", "medium","light")),
...
gp_oc <- read.csv("../output/data for sensitivity analysis scatter_gp_oc.csv", 
                     header = T) %>% 
  dplyr::mutate(
    scenario = factor(scenario, levels = c("BAU", "MIT", "CWW")),
    region = factor(region, levels = c("north", "central", "east")),
...
ww_pc <- read.csv("../output/data for sensitivity analysis scatter_ww_pc.csv", 
                     header = T) %>% 
  dplyr::mutate(
    scenario = factor(scenario, levels = c("BAU", "MIT", "CWW")),
    region = factor(region, levels = c("north", "central", "east")),
...
## 5.2. Figure 5(a): Opp costs, LD-HR fields
```{r Fig5a-plot-LDHR}
fig_5a <- ggplot(oc_fig5 %>% 
  ...
  labs(x = "Proportion of grid cells in very high density state", 
       y = "Mean annual opportunity cost, LD-HR fields, BAU-MIT (£/ha)") +
  theme(strip.placement = 'outside',
...
## 5.3. Figure 5(b): Opp costs, HD-HR fields
```{r Fig5b-plot-HDHR}
fig_5b <- ggplot(oc_fig5 %>% 
  ...
  labs(x = "Proportion of grid cells in very high density state", 
       y = "Mean annual opportunity cost, HD-HR fields, BAU-MIT (£/ha)") +
  theme(strip.placement = 'outside',
        #strip.background.x = element_blank(),
        #strip.text.x = element_blank(), #removes facet labels
        strip.text.x = element_text(size = 18),#, color = "red", face = "bold.italic"
...
## 8.2. Figure S6(a): Scatter plot, gp, MIT & BAU
### 8.2.1. Specify plot
...
### 8.2.2. Save plot
```{r FigS6a-save}
png("../figures/Supplementary Figure S6a.png",
    height = 10, width = 10, units = "in", res = 300)
print(figS6a)
dev.off()
```

## 8.3. Figure S6(b): Scatter plot, ww, MIT & BAU
### 8.3.1. Specify plot
```{r plot-ww-sensanaly-noCWW}
figS6b <- ggplot(ww_pc %>% filter(!scenario=="CWW"),
...
### 8.3.1. Save plot.
```{r FigS6b-save}
png("../figures/Supplementary Figure S6b.png",
    height = 10, width = 10, units = "in", res = 300)
print(figS6b)
dev.off()
```

...
Colour.
```{r}
png("../figures/Script 09_line graph_den and gp.png",
...
Black and white.
```{r}
png("../figures/Script 09_line graph_den and gp_bw.png",
    height = 6, width = 17, units = "in", res = 300)
...
## 3.4. Save
```{r}
png("../figures/Script 09_scatter_gp and wwyld.png",
...
```{r}
png("../figures/Script 09_scatter_gp and wwyld_subscripts.png",
    height = 10, width = 20, units = "in", res = 300)
...
## 4.4. Save
```{r}
png("../figures/Script 09_scatter_gp and wwyld_hd.png",
    height = 10, width = 20, units = "in", res = 300)
...
```{r}
png("../figures/Script 09_scatter_gp and wwyld_subscripts_hd.png",
    height = 10, width = 20, units = "in", res = 300)
...
Then specify the plot.  
Colour version.
```{r plot-scatter-worstcase-oc}
scatt_oc_current_worst <- ggplot(df_wc_oc, aes(initdenres_collapsed, oc*(-1), 
  ...
  scale_color_manual(name = "Situation",
                     values = c("#EECC66", # current, (capri blue #00C2F9) (blue #1A85FF)
...
Specify the plot. 
Colour.
```{r plot-scatter-worstcase-pc}
scatt_pc_current_worst <- ggplot(df_wc_pc, aes(initdenres_collapsed, pc*(-1), 
                    colour = situation, shape = situation)) + 
  facet_grid(region ~ soiltype,
             labeller = labeller(region = region.labs, 
                                 soiltype = soiltype.labs)
...
Colour.
```{r}
scatt_wc_oc_pc <- (scatt_oc_current_worst + scatt_pc_current_worst) + 
  plot_annotation(tag_levels = 'a', tag_prefix = '(',  tag_suffix = ')') & 
  theme(plot.tag = element_text(size = 18, face="bold"))
scatt_wc_oc_pc
```
...
Colour.
```{r}
png("../figures/Script 09_scatter_worstcase_oc and pc.png",
    height = 10, width = 20, units = "in", res = 300)
print(scatt_wc_oc_pc)
dev.off()


tiff("../figures/Script 09_scatter_worstcase_oc and pc.tiff", 
...
Black and white.
```{r}
png("../figures/Script 09_scatter_worstcase_oc and pc_bw.png",
    height = 10, width = 20, units = "in", res = 300)
...
# 6. Stacked bar plot
Colour.
```{r stacked-plot, echo=TRUE, fig.keep='all'}
sp <- ggplot(df_for_stack %>% dplyr::filter(!cost=="cly"),
             aes(x=xval, y=amount, fill=cost)) +
  geom_bar(position="stack", stat="identity") +
  scale_fill_manual(
    name = "Cost \ncomponent",
    values = c('#9ecae1', '#3ac8e1', '#005FCC'),# '#08306b',  #0E61DC
  ...
  # output with no BG
  geom_point(aes(x = 1.05, y = 1446.279), # point
             cex = 4, shape = 17, show.legend = FALSE) + 
  geom_segment(aes(x = 1.05, y = 1332.746, xend = 1.05, yend = 1559.811)) + # CIs
  # gross profit line
  ...
  # output with no BG
  geom_point(aes(x = 2.05, y = 1484.513), cex = 4, shape = 17, show.legend = FALSE) + # point
  geom_segment(aes(x = 2.05, y = 1367.219, xend = 2.05, yend = 1601.806)) + # CIs
  # gross profit line
  ...
  # 3. BAU HD-HR
  # output
  geom_point(aes(x = 3, y = 1349.2), cex = 4, show.legend = FALSE) + # point #colour = "#020E20",
  geom_segment(aes(x = 3, y = 1244.473, xend = 3, yend = 1453.913)) + # CIs
  # output with no BG
  geom_point(aes(x = 3.05, y = 1513.394), cex = 4, shape = 17, show.legend = FALSE) + 
  geom_segment(aes(   x = 3.05,    y = 1394.663, 
                   xend = 3.05, yend = 1632.125)) + # CIs #,colour = "#08306b"
  # gross profit line
  ...
  # output with no BG
  geom_point(aes(x = 4.05, y = 1456.491), cex = 4, shape = 17, show.legend = FALSE) + # point
  geom_segment(aes(x = 4.05, y = 1358.818, xend = 4.05, yend = 1554.165)) + # CIs
  ...
  # output with no BG
  geom_point(aes(x = 5.05, y = 1349.842), cex = 4, shape = 17, show.legend = FALSE) + # point
  geom_segment(aes(x = 5.05, y = 1267.995, xend = 5.05, yend = 1431.689)) + # CIs
  # gross profit from BAU scenario
  ...
  # output with no BG
  geom_point(aes(x = 6.05, y = 1246.424), cex = 4, shape = 17, show.legend = FALSE) + # point
  geom_segment(aes(x = 6.05, y = 1164.023, xend = 6.05, yend = 1328.824)) + # CIs
  # gross profit from BAU scenario
...
sp_bw <- ggplot(df_for_stack %>% dplyr::filter(!cost=="cly"), 
                aes(x=xval, y=amount, fill=cost)) +
  geom_bar(position="stack", stat="identity") +
  scale_fill_manual(
    name = "Cost \ncomponent",
...
Colour.
```{r}
png("../figures/Script 11_cost components_stacked.png",
...
Black and white.
```{r}
png("../figures/Script 09_cost components_stacked_bw.png",
    height = 10, width = 17, units = "in", res = 300)
print(sp_bw)
dev.off()
```
...
#### Save plot
```{r save-sp1}
#ggsave("../figures/Script 3c_prop hvh_yr6_effect of scenario and initcondit_3dr_scatter_noCWW.png", 
#       height = 10, width = 10, dpi = 300)

png("../figures/Script 3c_prop hvh_yr6_effect of scenario and initcondit_3dr_scatter_noCWW.png",
...
Save plot.
```{r, eval=FALSE, echo=FALSE}
#ggsave("../figures/Script 3c_prop hvh_yr6_effect of scenario and initDR_3dr_scatter_noCWW_VD added.png",
#       height = 10, width = 10)

png("../figures/Script 09_prop hvh_yr6_noCWW_LD-HR added.png",
    height = 10, width = 10, units = "in", res = 300)
print(sp2)
dev.off()
```
...
png("../figures/Script 09_SA_gp by initcondit_BAU & MIT.png",
    height = 10, width = 10, units = "in", res = 300)
print(scatt_SA_gp)
dev.off()
```
...
Save plot.
```{r save-econ-plot-noCWW}
png("../figures/Script 09_SA_ww by initcondit_BAU & MIT.png",
    height = 10, width = 10, units = "in", res = 300)
print(scatt_SA_ww)
dev.off()
```

...
econ_allimp_new <- inner_join(econ_allimp, 
                              ...
                              by = "ID") %>% 
  dplyr::rename(potential_output = output) %>% 
  dplyr::mutate(wtd_cyl = potential_output - wtd_output)
```
...
summarised_df_3DR <- econ_allimp_new %>% 
  ...
  dplyr::summarise(across(c(starts_with("wtd"),potential_output),.fns = list( 
  ...
  dplyr::mutate(
    ID = factor(paste(scenario, 
                      ...
                      substr(soiltype, 1, 1), # 1st letter of soil type
...
rotn_means <- econ_3DR %>% 
  ...
  dplyr::summarize(
    ...
    # potential output
    pot_out_mean = mean(potential_output_mean, na.rm = TRUE),
    pot_out_ci_l = mean(potential_output_mean, na.rm = TRUE) - 
      (1.96 * sd(potential_output_mean, na.rm = TRUE) / sqrt(length(potential_output_mean))),
    pot_out_ci_u = mean(potential_output_mean, na.rm = TRUE) + 
      (1.96 * sd(potential_output_mean, na.rm = TRUE) / sqrt(length(potential_output_mean)))
    ) %>% 
  ...
  dplyr::summarize(
    ...
    # output
    output_with_BG = mean(output_mean, na.rm = TRUE),
    output_with_BG_ci_l = mean(output_mean, na.rm = TRUE) - 
      (1.96 * sd(output_mean, na.rm = TRUE) / sqrt(length(output_mean))),
    output_with_BG_ci_u = mean(output_mean, na.rm = TRUE) + 
      (1.96 * sd(output_mean, na.rm = TRUE) / sqrt(length(output_mean))),
    
    # potential output (i.e. potential £ if there were no yield loss)
    output_without_BG = mean(pot_out_mean, na.rm = TRUE),
    output_without_BG_ci_l = mean(pot_out_mean, na.rm = TRUE) - 
      (1.96 * sd(pot_out_mean, na.rm = TRUE) / sqrt(length(pot_out_mean))),
    output_without_BG_ci_u = mean(pot_out_mean, na.rm = TRUE) + 
      (1.96 * sd(pot_out_mean, na.rm = TRUE) / sqrt(length(pot_out_mean)))
    ) %>% 
  dplyr::relocate(output_with_BG:output_without_BG_ci_u, .after = initdenres_collapsed)
```


# 5. Plot cost components

Get the data into a different format for plotting, and save the output so can plot elsewhere.
```{r}
